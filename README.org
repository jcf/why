...
*** Manifests aren't a reproducible panacea
What's in this ~metadata.json~ file anyway? Let's have a look.

file:graphql/metadata.json

Not enough information to spin up a full environment including PostgreSQL so we
have work to do.
